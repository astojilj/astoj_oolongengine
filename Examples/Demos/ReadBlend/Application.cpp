 ...
 *  PlayBlend
 *
 *  Created by astojilj@gmail.com on 7/12/11.
 *  Refactoring content from ReadBlender/Application.cpp file
 *
 */
...
#else
	GetResourcePathASCII(fullName, 512 - strlen(name) -1);
	sprintf(fullName,"%s/%s",fullName,name);
#endif
	file = fopen(fullName,"rb");

	if (!file) {
		printf("Exiting, file not found:%s\n", fullName);
		exit(0);
	}
...
*/
	...
	//blendReader->m_cameraTrans.getOpenGLMatrix(mCamera.f);
 ...
 */
static void modelTransform(MATRIX &mOut)
{
...
bool CShell::UpdateScene()
{
	...
	MatrixPerspectiveFovRH(MyPerspMatrix, f2vt(70), f2vt((WindowWidth / WindowHeight)), f2vt(1.0f), f2vt(150.0f), 0);
